...
async function exportLoadersAsync({
  ...
  debug('Bundling loaders for routes:', entryPoints.map((e) => e.page));
  ...
  const routesWithLoaders = new Set(entryPoints.map((e) => e.page));
  // NOTE(@hassankhan): We should ideally persist the manifest to `files` only once instead of
  ...
  debug('Exported loaders for routes:', entryPoints.map((e) => e.page));
}
